...
// Changes:
//  • SmartLink removed from form
//  • Country + State added as text inputs
//  • IP Type: Static IP / Dynamic IP mutually-exclusive checkboxes
//  • Auto-fetch Cache / Keywords / Smartlink from address_assignments when fingerprint loads
//  • White glassmorphism theme, crimson accent only
  ...
  return (
    <button
      ...
      style={{
        ...
        borderRadius:  6, color: copied ? "#059669" : "#666688",
        fontSize: 10, fontWeight: 600, padding: "3px 8px",
        cursor: "pointer", fontFamily: "inherit",
        transition: "all 0.18s cubic-bezier(0.16,1,0.3,1)",
        flexShrink: 0, letterSpacing: 0.3, marginLeft: 4,
        display: "inline-flex", alignItems: "center", gap: 4,
      }}
      onMouseEnter={(e) => { if (!copied) { e.currentTarget.style.background = "rgba(0,0,0,0.07)"; e.currentTarget.style.color = "#444466"; } }}
      onMouseLeave={(e) => { if (!copied) { e.currentTarget.style.background = "rgba(0,0,0,0.04)"; e.currentTarget.style.color = "#666688"; } }}
    >
      {copied ? "✓ Copied" : (
...
function CooldownTimer({ endsAt }: { endsAt: string }) {
  ...
  return (
    <div style={{ background: "rgba(244,63,94,0.05)", border: "1px solid rgba(244,63,94,0.14)", borderRadius: 8, padding: "10px 12px", display: "flex", flexDirection: "column", gap: 4 }}>
      ...
      <span style={{ color: "#aaaacc", fontSize: 10 }}>Expires: {new Date(endsAt).toLocaleString()}</span>
    </div>
  );
}

// ── ReadonlyField — displays auto-fetched values with copy ────────────────────
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
        <div style={{
          flex: 1, background: "rgba(142,22,22,0.04)",
          border: "1px solid rgba(142,22,22,0.12)",
          borderRadius: 9, padding: "8px 12px",
          color: "#1a1a2e", fontSize: 12.5,
...
export default function AddRecordModal({ onClose, onSaved }: AddRecordModalProps) {
  ...
  const [attached,    setAttached]    = useState<AttachedData | null>(null);
  const [fetchingMeta, setFetchingMeta] = useState(false);

  const confirmingRef = useRef(false);
  const validation    = useValidation();

  // Auto-fill fingerprint when known
  useEffect(() => {
  ...
  // Auto-fetch Cache / Keywords / Smartlink locked to this IP
  useEffect(() => {
    if (!validation.result.known_fingerprint || !uniqueId.trim()) {
      setAttached(null);
      return;
    }
    let cancelled = false;
    ...
    supabase
      ...
      .select("cache, keywords, smartlink")
      .eq("address_text", uniqueId.trim())
      .eq("locked_meta", true)
      .order("assigned_at", { ascending: false })
      .limit(1)
      .then(({ data }) => {
        if (cancelled) return;
        const row = (data as AttachedData[] | null)?.[0];
        setAttached(row ?? null);
        setFetchingMeta(false);
      });
    return () => { cancelled = true; };
  }, [validation.result.known_fingerprint, uniqueId]);

  function handleUidChange(val: string) {
    setUniqueId(val);
    setConfirmed(false); setConfirmMsg(null); setAttached(null);
    if (val.trim()) validation.validate(val, fingerprint, ADMIN_ASSIGNMENT_ID);
    else validation.reset();
  }

  function handleFpChange(val: string) {
    setFingerprint(val);
    setConfirmed(false); setConfirmMsg(null);
    if (uniqueId.trim()) validation.validate(uniqueId, val, ADMIN_ASSIGNMENT_ID);
    else validation.reset();
  }
  ...
  async function handleConfirm() {
    ...
    if (!uniqueId.trim() || !fingerprint.trim()) {
      setConfirmMsg({ ok: false, text: "Enter Unique ID and Fingerprint before confirming." }); return;
    }
    const st = validation.result.status;
    if (st === "checking") { setConfirmMsg({ ok: false, text: "Validation still running — please wait." }); return; }
    if (st === "idle" || st === "error") {
      validation.validate(uniqueId, fingerprint, ADMIN_ASSIGNMENT_ID);
      setConfirmMsg({ ok: false, text: "Running live check — wait, then click Confirm again." }); return;
    }
    if (st === "blocked") { setConfirmMsg({ ok: false, text: validation.result.message ?? "Identity in cooldown." }); return; }
    if (st === "mismatch") { setConfirmMsg({ ok: false, text: validation.result.message ?? "Fingerprint mismatch." }); return; }
    ...
    setConfirmMsg({ ok: true, text: "Verified — identity is clear. You may now save." });
  }

  async function handleSave() {
    if (!category || !uniqueId.trim() || !fingerprint.trim()) {
      setError("Category, Unique ID, and Fingerprint are required."); return;
    }
  ...
  const hasAttached = attached && (attached.cache || attached.keywords || attached.smartlink);
    ...
    <div style={M.overlay}>
      <div style={M.modal}>
        ...
        <div style={M.body}>
            ...
            <div style={M.field}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                {(["Static IP", "Dynamic IP"] as IpType[]).map((type) => {
                  ...
                  return (
                    <button key={type} type="button" onClick={() => setIpType(active ? "" : type)}
                      style={{
                        ...
                        transition: "all 0.18s cubic-bezier(0.16,1,0.3,1)",
                        boxShadow: active ? "0 2px 10px rgba(142,22,22,0.12)" : "0 1px 3px rgba(0,0,0,0.05)",
                      }}
                      onMouseEnter={(e) => { if (!active) { e.currentTarget.style.borderColor = "rgba(142,22,22,0.20)"; e.currentTarget.style.color = "#8e1616"; e.currentTarget.style.background = "rgba(142,22,22,0.04)"; } }}
                      onMouseLeave={(e) => { if (!active) { e.currentTarget.style.borderColor = "rgba(0,0,0,0.10)";     e.currentTarget.style.color = "#8888aa"; e.currentTarget.style.background = "rgba(255,255,255,0.60)"; } }}
                    >
                      <span style={{ width: 14, height: 14, borderRadius: "50%", border: `2px solid ${active ? "#8e1616" : "rgba(0,0,0,0.22)"}`, display: "flex", alignItems: "center", justifyContent: "center", transition: "border-color 0.18s ease", flexShrink: 0 }}>
                        {active && <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#8e1616", animation: "scaleIn 0.18s cubic-bezier(0.16,1,0.3,1) both" }} />}
                      </span>
                      {type}
                    </button>
                  );
          ...
          <div style={M.row}>
            <div style={M.field}>
              <label style={M.label}>Country</label>
              <input
                style={M.input} value={country}
            ...
            <div style={M.field}>
              <label style={M.label}>State</label>
              <input
                style={M.input} value={state}
            ...
            <div style={M.field}>
              ...
              <div style={M.inputRow}>
                <input
                  ...
                  onFocus={(e) => { if (vStatus === "idle") { e.currentTarget.style.borderColor = "rgba(142,22,22,0.40)"; e.currentTarget.style.boxShadow = "0 0 0 3px rgba(142,22,22,0.07)"; } }}
                  onBlur={(e)  => { if (vStatus === "idle") { e.currentTarget.style.borderColor = "rgba(0,0,0,0.10)";    e.currentTarget.style.boxShadow = "none"; } }}
                />
            ...
            <div style={M.field}>
              ...
              <div style={M.inputRow}>
                <input
                  style={{
                    ...M.input,
                    borderColor:  validation.result.known_fingerprint ? "rgba(142,22,22,0.28)" : isMismatch ? "rgba(245,158,11,0.40)" : "rgba(0,0,0,0.10)",
                    background:   validation.result.known_fingerprint && !confirmed ? "rgba(142,22,22,0.04)" : "#ffffff",
                  }}
                  value={fingerprint}
                  onChange={(e) => handleFpChange(e.target.value)}
                  onKeyDown={handleKeyDown}
                  placeholder="e.g. FP-A4B2C"
                  onFocus={(e) => { if (!validation.result.known_fingerprint) { e.currentTarget.style.borderColor = "rgba(142,22,22,0.40)"; e.currentTarget.style.boxShadow = "0 0 0 3px rgba(142,22,22,0.07)"; } }}
                  onBlur={(e)  => { if (!validation.result.known_fingerprint) { e.currentTarget.style.borderColor = "rgba(0,0,0,0.10)";    e.currentTarget.style.boxShadow = "none"; } }}
                />
          ...
          {hasAttached && !fetchingMeta && (
            <div style={{
              ...
              <ReadonlyField label="Keywords"  value={attached!.keywords} />
              <ReadonlyField label="Smartlink" value={attached!.smartlink} />
            </div>
          )}
          ...
          {vStatus !== "idle" && (
            <div style={{
              ...
              {validation.result.message && !isChecking && (
                <span style={{ color: "#8888aa", fontSize: 11.5, lineHeight: 1.5 }}>{validation.result.message}</span>
              )}
              {isBlocked && validation.result.cooldown_ends_at && <CooldownTimer endsAt={validation.result.cooldown_ends_at} />}
              {isMismatch && validation.result.expected_fingerprint && (
                <div style={{ background: "rgba(245,158,11,0.04)", border: "1px solid rgba(245,158,11,0.12)", borderRadius: 8, padding: "8px 12px", display: "flex", flexDirection: "column" as const, gap: 3 }}>
                  <span style={{ color: "#8888aa", fontSize: 10, fontWeight: 500 }}>Bound fingerprint:</span>
                  <code style={{ color: "#d97706", fontSize: 11.5, fontFamily: "'JetBrains Mono', monospace" }}>{validation.result.expected_fingerprint}</code>
                </div>
              )}
              {isValid && validation.result.known_fingerprint && (
                <div style={{ background: "rgba(5,150,105,0.04)", border: "1px solid rgba(5,150,105,0.12)", borderRadius: 8, padding: "8px 12px", display: "flex", alignItems: "center", gap: 8 }}>
                  <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 3 }}>
                    <span style={{ color: "#8888aa", fontSize: 10, fontWeight: 500 }}>Recalled fingerprint:</span>
                    <code style={{ color: "#059669", fontSize: 11.5, fontFamily: "'JetBrains Mono', monospace" }}>{validation.result.known_fingerprint}</code>
                  </div>
          ...
          {showConfirmBtn && (
            <button
              ...
              style={{
                background:     confirming ? "rgba(142,22,22,0.25)" : "linear-gradient(135deg, #8e1616 0%, #6b1010 100%)",
                border: "none", borderRadius: 9, color: "#fff", fontSize: 12.5, fontWeight: 600,
                padding: "10px 0", cursor: confirming || isChecking || isBlocked ? "not-allowed" : "pointer",
                fontFamily: "inherit", width: "100%",
                display: "flex", alignItems: "center", justifyContent: "center", gap: 6,
                letterSpacing: 0.2, boxShadow: confirming ? "none" : "0 4px 14px rgba(142,22,22,0.28)",
...
const M: Record<string, React.CSSProperties> = {
  ...
  header: {
    ...
    padding: "16px 22px", borderBottom: "1px solid rgba(0,0,0,0.07)",
  },
  headerTitle: { color: "#1a1a2e", fontWeight: 700, fontSize: 16, letterSpacing: -0.2 },
  closeBtn: {
    background: "transparent", border: "none", borderRadius: 7,
    color: "#8888aa", cursor: "pointer", fontSize: 15, lineHeight: 1,
    padding: "5px 7px", transition: "all 0.18s ease",
    display: "flex", alignItems: "center", justifyContent: "center",
  },
  body:     { padding: "18px 22px", display: "flex", flexDirection: "column", gap: 13 },
  ...
  label: {
    color: "#8888aa", fontSize: 10, letterSpacing: 0.8,
    textTransform: "uppercase", display: "flex", alignItems: "center", gap: 4, fontWeight: 600,
  },
  input: {
    background: "#ffffff", border: "1px solid rgba(0,0,0,0.10)", borderRadius: 9,
    color: "#1a1a2e", padding: "9px 12px", fontSize: 13,
    fontFamily: "inherit", outline: "none", flex: 1, width: "100%",
    transition: "border-color 0.18s ease, box-shadow 0.18s ease",
  },
  kbd: {
    background: "rgba(0,0,0,0.05)", border: "1px solid rgba(0,0,0,0.12)", borderRadius: 5,
    padding: "1px 6px", fontSize: 11, color: "#444466", fontFamily: "'JetBrains Mono', monospace",
  },
  infoBox: {
    background: "rgba(142,22,22,0.04)", border: "1px solid rgba(142,22,22,0.12)",
    color: "#7a4040", borderRadius: 9, padding: "11px 14px", fontSize: 12, lineHeight: 1.65,
  },
  errorBox: {
    background: "rgba(244,63,94,0.05)", border: "1px solid rgba(244,63,94,0.18)",
    color: "#f43f5e", borderRadius: 8, padding: "10px 12px", fontSize: 12,
  },
  footer: {
    display: "flex", justifyContent: "flex-end", gap: 10,
    padding: "14px 22px", borderTop: "1px solid rgba(0,0,0,0.07)",
  },
  btnCancel: {
    background: "transparent", border: "1px solid rgba(0,0,0,0.10)", color: "#8888aa",
  ...
  btnSave: {
    ...
    border: "none", color: "#fff", borderRadius: 9, padding: "9px 22px",
    fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "inherit",
    boxShadow: "0 4px 14px rgba(142,22,22,0.28)",
    transition: "all 0.18s cubic-bezier(0.16,1,0.3,1)",
  },
  btnSaveDisabled: {
    background: "rgba(0,0,0,0.05)", border: "1px solid rgba(0,0,0,0.08)", color: "#bbbbcc",
    borderRadius: 9, padding: "9px 22px", fontSize: 13, fontWeight: 600,
    cursor: "not-allowed", fontFamily: "inherit", boxShadow: "none",
  },
};
